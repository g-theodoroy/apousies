function check_unique_key(field){
    ...
    with (field)
    {
        ...
        {
            if (value ==  pinakas[x] && value != document.getElementById("oldamget").value){
                eleghos = false;
            }
        }
        }
...
function validate_form(thisform)
{
    with (thisform)
    {
        ...
        var strAmCheckMsg = "Παρακαλώ να μη χρησιμοποιείτε τους χαρακτήρες '|' , '&' , '<' , '>'.";
        ...
        if (am_digit_check(email)==false)
        {
            alert(strAmCheckMsg);
            email.focus();
            return false;
        }
        if (am_digit_check(tmima)==false)
        {
            alert(strAmCheckMsg);
            tmima.focus();
            return false;
        }
        if (am_digit_check(tmima-kat)==false)
        {
            alert(strAmCheckMsg);
            tmima-kat.focus();
            return false;
        }
        if (am_digit_check(tmima-epi)==false)
        {
            alert(strAmCheckMsg);
            tmima-epi.focus();
            return false;
